<?php
/**
 * medcare functions and definitions
 *
 ...
 */
function medcare_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on medcare, use a find and replace
		* to change 'medcare' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'medcare', get_template_directory() . '/languages' );
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'medcare_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'medcare_setup' );
 ...
 */
function medcare_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'medcare_content_width', 640 );
}
add_action( 'after_setup_theme', 'medcare_content_width', 0 );
...
add_action( 'widgets_init', 'medcare_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function medcare_scripts() {
	wp_enqueue_style( 'medcare-style', get_stylesheet_uri(), array(), time() );
	//Bootstrap link - css
...
add_action( 'wp_enqueue_scripts', 'medcare_scripts' );
